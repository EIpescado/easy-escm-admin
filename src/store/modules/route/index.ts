...
export const useRouteStore = defineStore(SetupStoreId.Route, () => {
  ...
  async function initConstantRoute() {
    ...
    // constant routes are always defined in the front-end (login/403/404/500/iframe-page)
  ...
  async function initDynamicAuthRoute() {
    ...
    if (!error) {
      const routes = transformMenuTreeToRoutes(data || []);

      addAuthRoutes(routes);

      handleConstantAndAuthRoutes();

      const home: LastLevelRouteKey = (routeHome.value || 'home') as LastLevelRouteKey;
  ...
  async function getIsAuthRouteExist(routePath: RouteMap[RouteKey]) {
    ...
    // in dynamic mode, check whether the route is generated by elegant-router
    return Boolean(getRouteName(routePath));
  }
